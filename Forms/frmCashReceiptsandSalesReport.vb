...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.lvOrders.Size = New System.Drawing.Size(1182, 542)
        ...
        Me.Panel1.Size = New System.Drawing.Size(1182, 65)
        ...
        Me.Panel2.Size = New System.Drawing.Size(1182, 56)
        ...
        Me.ClientSize = New System.Drawing.Size(1182, 663)
...
#End Region
    ...
    Friend dateStart As Date = Today.Date
    ...
    Private Sub calculateTaxDistrictData(ByRef job2 As JobCollection.JobDetail)
        ...
        If Not (sumTaxes = Decimal.Round(CDec(job2.JobTaxRate), 2)) Then
            Dim strErr As String = "Job tax rate does not equal districts selected, please edit job: "
            Try
                strErr = String.Concat(strErr, job2.Customer, " ", job2.Reference)
            Catch
            End Try
        ...
        Try
            ...
            With oWord.Selection
                ...
                '.ParagraphFormat.TabStops.Add(Position:=oWord.Application.InchesToPoints(tabLength), Alignment:=Word.WdTabAlignment.wdAlignTabLeft, Leader:=Word.WdTabLeader.wdTabLeaderSpaces)
                    ...
                    .Font.Size = 8
                oWord.Selection.Font.Name = "Courier"
                    Dim strHeader As String = "DAY" & vbTab & "TaxableSale" & vbTab & "TAXID$" & vbTab & "Reference" _
                    ...
                    For Each tax1 As TaxDistrict In Me.alTaxDistrictColumns 'StateRTDColsOrdered
                        strHeader = String.Concat(strHeader, vbTab, tax1.DistrictName)
                    Next
                    strHeader = NormalizeColumnWidths(strHeader)
                ...
                .TypeText(strHeader)
                    ...
                    .Font.Size = 8
                        strline = strline & (job1.Day.ToString & vbTab)
                        ...
                        strline = strline & (job1.Reference.ToString & vbTab)
                        strline = strline & (job1.Customer.ToString & vbTab)
                        ...
                        For k = 0 To Me.alTaxDistrictColumns.Count - 1 ' StateRTDColsOrdered.Count - 1
                            oTaxDistA = Me.alTaxDistrictColumns(k) ' StateRTDColsOrdered(k)
                            Dim oTaxDist1 As TaxDistrict
                            Dim blnFoundColumn As Boolean
                            For i = 0 To job1.alTaxDistrict.Count - 1
                                oTaxDist1 = job1.alTaxDistrict(i)
                        ...
                        strline = NormalizeColumnWidths(strline)
                        .TypeText(strline)
                    ...
                    Dim strlineTotals As String = String.Concat(vbTab, Format(Me.JobCollection1.TotalSales, "currency"), _
                    ...
                    Dim j As Long
                    For j = 0 To Me.alTaxDistrictColumns.Count - 1 ' StateRTDColsOrdered.Count - 1
                        oTaxDist = Me.alTaxDistrictColumns(j) ' StateRTDColsOrdered(j)
                        strlineTotals = String.Concat(strlineTotals, vbTab, Format(oTaxDist.reportSumAmount, "currency"))
                    Next

                    strlineTotals = NormalizeColumnWidths(strlineTotals)

                .TypeText(strlineTotals)
            ...
            Try
                ...
                Try
                    Dim sVer As String = oWord.Version.ToString
                    If Not sVer.Contains("11.0") Then
                        Dim noSpacingStyle As Object = "No Spacing"
                        oWord.ActiveWindow.Selection.Style = (noSpacingStyle)
                    End If
                Catch ex As Exception
    ...
    Private Function NormalizeColumnWidths(strFull As String) As String
        Dim fnPadTrim = Function(str1 As String, iwidth As Integer) As String
                            Return Mid(str1.PadRight(20, " "), 1, iwidth) & " "
                        End Function
        Dim list1 As List(Of String) = strFull.Split(vbTab).ToList
        Dim icount As Integer = 0
        Dim strReturn As String = ""

        For icount = 0 To list1.Count - 1 ' Each item1 In list1
            Dim item1 As String = list1(icount)
            Select Case icount
                Case 0 'day
                    item1 = fnPadTrim(item1, 6)
                Case 1 'taxsale
                    item1 = fnPadTrim(item1, 13)
                Case 2 'taxid$
                    item1 = fnPadTrim(item1, 12)
                Case 3 ' ref
                    item1 = fnPadTrim(item1, 12)
                Case 4 'cust
                    item1 = fnPadTrim(item1, 20)
                Case 5, 6, 7 ' chg ar 4010 etc state rtd
                    item1 = fnPadTrim(item1, 12)
                Case Else
                    item1 = fnPadTrim(item1, 11)
            End Select
            list1(icount) = item1
        Next
        Return String.Concat(list1)
    ...
    Private Sub DisplayInWordPAD()
        ...
        Try
            ...
            Dim strHeader As String = "DAY" & vbTab & "TaxableSale" & vbTab & "TAXID$" & vbTab & "Reference" _
                    & vbTab & "Customer Name" & vbTab & "ChargeA/R" & vbTab & "4010" & vbTab _
                    & "4070" & vbTab & "4020" & vbTab & "4030" & vbTab & "4040" & vbTab _
                    & "TAX"
            ...
            For Each Str As TaxDistrict In Me.alTaxDistrictColumns ' StateRTDColsOrdered
                strHeader = String.Concat(strHeader, vbTab, Str.DistrictName)
            Next
            strHeader = NormalizeColumnWidths(strHeader)
            ...
            For Each job1 In JobCollection1.JobDetailList
                ...
                strLine = (job1.Day.ToString & vbTab)
                ' strLine = strLine & (vbTab)
                strLine = strLine & (Format((job1.TaxableSale.ToString), "currency") & vbTab)
                strLine = strLine & (Format((job1.NonTaxable.ToString), "currency") & vbTab)
            ...
            strLine = (vbTab & Format(Me.JobCollection1.TotalSales, "currency") & _
            vbTab & Format(Me.JobCollection1.TotalNonTaxable, "currency") & vbTab & vbTab & _
            vbTab & Format(Me.JobCollection1.TotalAR, "currency") & vbTab & _
                Format(Me.JobCollection1.Total4010, "currency") & vbTab & _
                Format(Me.JobCollection1.Total4070, "currency") & vbTab & _
